...
function Table({ solicitacoes, selected, onCheckboxChange, onSelectAllChange, selectAll, onSuccess }: TableProps) {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      await axios.post(
        'http://44.205.22.49:8080/api/depoimento/aprovar', 
        [id],
        {
          headers: {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      await axios.post(
        'http://44.205.22.49:8080/api/depoimento/reprovar', 
        [id],
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      <table className="w-full table-auto border-collapse">
        <thead>
          <tr className="bg-[#F1FBFF] text-xl">
            ...
            <th className="w-10/20 px-2 py-4 border border-[#000] max-[900px]:hidden">Depoimento</th>
