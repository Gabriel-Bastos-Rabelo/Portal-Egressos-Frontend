...
function Table({ solicitacoes, selected, onCheckboxChange, onSelectAllChange, selectAll, onSuccess }: TableProps) {
  ...
  const [selectedSolicitacao, setSelectedSolicitacao] = useState<Oportunidade | null>(null);
  const [success, setSuccess] = useState(false);
  const [reject, setReject] = useState(false);
  
  const openModal = (solicitacao: Oportunidade) => {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      await axios.post(
        'http://44.205.22.49:8080/api/oportunidade/aprovar', 
        [id],
        {
          headers: {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      await axios.post(
        'http://44.205.22.49:8080/api/oportunidade/reprovar', 
        [id],
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  return (
    <>
      ...
      <table className="w-full table-auto border-collapse">
        <thead>
          <tr className="bg-[#F1FBFF] text-xl">
            ...
            <th className="w-6/20 px-2 py-4 border border-[#000]">Título da Oportunidade</th>
        ...
        <tbody>
          {solicitacoes.map((solicitacao, index) => (
            <tr key={index} className="border-b border-[#000] text-x1">
              ...
              <td className="px-2 py-4 font-bold text-center text-lg">{solicitacao.titulo}</td>
      ...
      {isModalOpen && selectedSolicitacao && (
        <OportunidadeModal
          oportunidade={selectedSolicitacao}
          onClose={closeModal}
        />
      )}
    </>
